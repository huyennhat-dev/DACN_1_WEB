          ...
          <template v-if="column.key == 'status'">
            <div class="d-flex justify-content-center">
              <a-button
                :class="{
                  'ant-btn-primary':
                    record.orderStatus.slug == 'dang-van-chuyen',
                  'ant-btn-dangerous':
                    record.orderStatus.slug == 'da-nhan-hang',
                }"
                ghost
                class="brr-5"
              >
...
export default defineComponent({
  ...
  setup() {
    useMenuStore().onSelectedKeys(["admin-orders-1"]);
  ...
  methods: {
    ...
    async getOrders() {
      try {
        const res = await axios.get(`${BASE_URL}/admin/orders/orders-1`, {
          headers: {
